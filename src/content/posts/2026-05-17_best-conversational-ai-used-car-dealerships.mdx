...
The right tool depends on where the leak actually is. If shoppers are bouncing off vehicle detail pages before they contact the BDC, you need an inventory-aware web assistant. If after-hours leads are going cold, you need phone and SMS coverage. If the problem is inside the CRM - AI on every lead, not just the new ones - that's a different product category entirely. These five cover the tools used-car GMs actually put on the shortlist:

- **[Nobi](https://nobi.ai)** - AI sales assistant for the dealership website with inline source citations and an optional second AI review on every answer. Starts at $25/month base. Pick when shoppers bounce off VDPs because they can't get fast, accurate answers about specific vehicles.
- **[Impel](https://impel.ai)** - Automotive AI Operating System spanning lead engagement, service scheduling, and lifecycle marketing. Quote-only; per-rooftop or dealer-group pricing. Pick when a dealer group is consolidating onto one AI vendor across sales, service, and marketing.
- **[DriveCentric](https://drivecentric.com)** - AI-first automotive CRM with autonomous AI agents and multi-channel follow-up native to the lead record. Quote-only; per-rooftop pricing scaled by units sold. Pick when the real problem is a tired CRM and you want AI on every lead by default.
- **[Matador AI](https://matador.ai)** - Conversational AI across phone, text, email, social, and web chat with a built-in TCPA compliance engine. Quote-only; per-rooftop pricing. Pick when SMS and phone follow-up are where leads actually convert.
- **[DealerAI](https://dealerai.com)** - Multi-agent platform for website plus Meta plus SMS with published per-site tiers from $599/month. Pick when you want a dealership-native chat layer without a CRM swap and need a number you can budget against.
...
Grounding matters. The Chevrolet of Watsonville $1 Tahoe story is the cautionary tale every dealer cites: an ungrounded bot agreed to sell a Tahoe for a dollar. Generic AI tools routinely fail on automotive specifics - quoting stale inventory, getting trim details wrong, confusing year/make/model combinations; automotive-specific tools grounded in the dealer's own data hold up better on shopper accuracy.
...
- The second-pass AI review blocks answers it can't verify against the cited sources - the guard against quoting a trim package, a price, or a vehicle status that's no longer accurate.
