...
Product results show the matched SKU with image, title, and price. For high-confidence matches, one tap takes the shopper to the PDP - no full results page in between. Suggested query completions give the shopper a one-tap way to run a fuller search when the right product isn't already showing.
