export const meta = {
 ...
 answer: "Those signal problems go deeper than ranking. A B2B search layer needs to resolve part-number synonyms, handle special characters in SKUs, surface the right variant across a deep attribute hierarchy, and answer pre-purchase questions from connected spec sheets and policy docs - all without routing the buyer to a rep."
 },
 {
 question: "What is Nobi, and how does it address B2B catalog search?",
 answer: "Nobi is a conversational website assistant that combines semantic product search, automated Q&A, and lead capture in one product. For B2B catalogs, that pairing covers both layers of the problem: search returns the right part, and when a buyer follows up with a question about lead time or minimum order quantity, the assistant answers from your connected spec sheets and policy docs rather than routing them to a sales rep."
 },
 {
 ...
 answer: "The decision comes down to three variables: how much engineering capacity you have to own implementation and ongoing maintenance, whether your primary gap is faceted filtering or conversational Q&A, and whether search needs to work across multiple surfaces or just your site. Each tool above was built for a different answer to those questions."
 },
 {
 question: "Frequently asked questions about B2B search",
 answer: "B2B operators evaluating search tools ask similar questions about part-number matching, account-specific pricing, and implementation overhead. The tool you pick from the decision framework above shapes the answers - but these questions come up regardless of which direction you go."
 }
 ],
};
...
On the search side, the matching is semantic - meaning over exact string. A query for "M10 flange nut grade 8.8 DIN 6923" returns the right result even when the catalog title uses different delimiter conventions or abbreviations. Format variation doesn't cause a miss.
...
Algolia and Hawk Search are engineering projects measured in weeks to months depending on catalog complexity. Nobi can index an existing catalog and start answering questions in hours from a product feed URL or uploaded spec sheets.
