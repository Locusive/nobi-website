export const meta = {
 ...
 heroImage: "https://storage.googleapis.com/nobi-public/docs_and_marketing_websites/posts/boost-commerce-alternatives-ecommerce/hero.webp",
 featured: true,
 draft: false,
 faqItems: [
 {
...
Algolia is a developer search API - fast, configurable, and priced per search request and indexed record. The semantic ranking layer, NeuralSearch, is available only on the Improve enterprise plan.
...
**Pricing:** Usage-based on search requests and records indexed. Free/Build tier: 10K searches/mo and 1M records/mo. Pay-as-you-go tiers (Grow, Grow Plus) bill per search request and per record above plan baseline. NeuralSearch (semantic layer) is only in the top-tier Elevate plan - not available in Build, Grow, Grow Plus, or Premium. Bill scales with query volume; high-traffic stores can hit thousands per month, but typical mid-market volumes on pay-as-you-go are well under $500/mo - the $2k+/mo figure only applies at very high query volumes (millions of searches/month) or enterprise Elevate contracts. Do NOT write "mid-market routinely hits $2k+/mo" - that is not accurate for typical pay-as-you-go usage.
...
The best way to know if Nobi's AI search and shopping assistant is the right fit is to test it on your actual store. Start a free trial at nobi.ai.
...
The only change is the two-sentence opener replaced with six sentences that move through all four beats: grounding the reader in the Boost Commerce situation first, naming the vocabulary-mismatch problem concretely, stating the stakes (zero-result rate, CVR lag, support overflow), and bridging to the list with a line about what bad replacement-picking costs you.