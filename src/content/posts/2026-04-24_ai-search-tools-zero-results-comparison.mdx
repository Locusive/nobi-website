export const meta = {
 ...
 {
 question: "1. Nobi",
 answer: "Nobi keeps the zero-result page from happening in the first place. Its AI-based semantic search returns results based on what shoppers mean, not just what words they typed - so 'brown boots' finds the Cappuccino Boot at Kilte even though 'brown' doesn't appear on the product page. Misspelled, vague, or unusual queries still return relevant products instead of an empty page. There's also a conversational assistant on the same screen for shoppers who want to ask a question rather than search. Kilte saw a 21.7% conversion-rate lift over Shopify's default search."
 },
 {
 question: "2. Algolia",
...
Nobi keeps the zero-result page from happening in the first place. Its [semantic search engine](https://nobi.ai/blog/how-ai-site-search-works) ranks based on what shoppers mean, not just what words they type. "Brown boots" finds Kilte's Cappuccino Boot even though "brown" never appears on the product page. Misspelled, vague, or unusual queries still come back with relevant products instead of an empty page. There's also a conversational assistant on the same screen for shoppers who want to ask a question instead of searching - "what goes with this?", "do you have this in a size 10?", "is this dishwasher safe?" - and the shopper gets a real answer pulled from the catalog. The empty results page just doesn't happen.
...
- Reports show zero-result queries (and low-relevance ones the engine catches before they hit zero) so the merch team can spot patterns. UNTUCKit reviews these in a weekly meeting and feeds the patterns back into their catalog.
...
If empty results pages are quietly costing you sales, see how Nobi keeps them from happening - and gives shoppers a way to ask follow-up questions on the same search screen - at nobi.ai.
