export const meta = {
 ...
 answer: "Nobi answers from your connected inventory feed, which refreshes twice a day. Grounding in that feed leaves less room for error than a model working from memory, though no AI system can guarantee it's never wrong. In practice, the most common cause of a wrong answer is the feed itself lagging a sale or price change between refreshes."
 },
 {
 question: "Does this replace our BDC?",
 answer: "Nobi captures the request and routes it with the vehicle attached. Your BDC still makes the call, owns the calendar, and closes the deal."
 },
 {
...
A [grounded assistant](/blog/best-conversational-ai-used-car-dealerships) that answers real questions and hands off cleanly carries a different risk than a scripted bot that loops. The qualification step here is deliberately short, just two questions, to avoid the back-and-forth that generates complaints.


<BlogEmailCapture hook="Letting high-intent VDP visitors leave without requesting a test drive?" subtext="Drop your email and we'll show you how Nobi captures test-drive requests right on the VDP and routes them to your CRM - no generic lead form standing in the way." />
