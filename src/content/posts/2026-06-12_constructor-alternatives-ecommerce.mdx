...
- **Klevu**, AI-powered Shopify site search with a no-code merchandising dashboard, quote-only (~$499–$1,598/mo). Pick when vocabulary mismatch is your primary CVR leak and you need occasional manual pinning on top.
- **Algolia**, search API with full developer control and sub-50ms response times, usage-based from a free tier. Pick when a dedicated engineering team wants exact control over how every product attribute factors into ranking.
- **Fast Simon**, Shopify-tuned visual merchandising and search toolkit, free starter (100 sessions/mo) then from $39.99/mo. Pick when collection curation and seasonal campaign layouts are the primary bottleneck, not semantic search.
- **Searchspring**, rule-based ecommerce search and merchandising, quote-only (mid-market tiered pricing; no published figures). Pick when the merchandising team wants total, auditable control over every query result with no black-box decisions.
