...
export default function ScrollPreview({
  ...
  return (
    ...
    >
      {preview && (
        <div
          className="pointer-events-auto flex max-w-lg items-center justify-center gap-3 rounded-full border border-black/10 bg-white/95 px-4 py-2 shadow-lg shadow-black/10 backdrop-blur dark:border-white/10 dark:bg-black/80"
          role="button"
          tabIndex={0}
        ...
        >
          <span className="text-[10px] uppercase tracking-[0.4em] text-black/50 dark:text-white/60">
            {label}
          </span>
          <span className="text-sm font-semibold text-black dark:text-white whitespace-nowrap">
            {preview.label}
          </span>
          <ChevronDown className="h-4 w-4 text-black/60 dark:text-white/70" />
        </div>
      )}
    </div>
  );
