...
[Fast Simon](https://fastsimon.com) is Shopify search with product recommendations and visual merchandising tools in one package. The visual merchandising piece is what earns it a spot on a jewelry shortlist: birthstone edits, anniversary capsules, and seasonal gift collections are visual-curation work as much as search work, and Fast Simon gives merchandisers a toolkit to manage those layouts without an engineering ticket. Pinning, boosting, and collection rules all live in the same dashboard, and the App Store install means a jewelry store is typically live within a day. The trade-off is that [natural-language understanding](https://nobi.ai/blog/ai-vs-traditional-site-search) is lighter than the AI-native engines above, so descriptive jewelry queries like "cushion-cut pink sapphire in yellow gold" still leak.
